...
mesh_res_list <- future_map(unique(responses),
                            ...
                            function(resp) {
                              ...
                              for (i in 1:nrow(mesh_grid)) {
                                ...
                                formula <- as.formula(paste0(resp, " ~ percent_population_growth_scaled*mean_density_km2 +
                             months_severe_drought_scaled +
                             fire_frequency_scaled +
                             mat_change_scaled + 
                             n_deposition_scaled"))
...
dt_mesh_res <- rbindlist(mesh_res_list) %>% 
  ...
  clean_term = case_when(
    ...
    term == "mat_scaled" ~ "MAT", 
    term == "map_scaled" ~ "MAP", 
    term == "percent_population_growth_scaled:mean_density_km2" ~ "Mean Ele. Density:Ele. Pop. Growth",
    term == "mean_density_km2:months_severe_drought_scaled" ~ "Mean Ele. Density:Drought Months",
    term == "mean_density_km2:fire_frequency_scaled" ~ "Mean Ele. Density:Fire Frequency",
    term == "mat_change_scaled:mat_scaled" ~ "MAT Trend:MAT"))
unique(dt_mesh_res$clean_term)
summary(dt_mesh_res)
fwrite(dt_mesh_res, "builds/model_outputs/sdmtmb_results_100m_mean_density.csv")
...
p_covs <- dt_mesh_res %>% 
  ...
  theme(legend.position = "none", 
        ...
        strip.background = element_rect(fill = "linen", color = "linen"))
p_covs
ggsave(plot = p_covs, "builds/plots/supplement/cov_estimates_different_meshs_100m_mean_density.png", dpi = 600, height = 12, width = 12)
...
p_est <- dt_mesh_res %>% 
  ...
  theme(legend.position = "none", 
        ...
        strip.background = element_rect(fill = "linen", color = "linen"))
p_est
ggsave(plot = p_est, "builds/plots/cov_estimates_best_mesh_100m_mean_density.png", dpi = 600, height = 6.5, width = 10)
...
p_cpo <- dt_mesh_res %>% 
  ...
  theme(legend.position = "right", 
        ...
        strip.background = element_rect(fill = "linen", color = "linen"))
p_cpo
ggsave(plot = p_cpo, "builds/plots/supplement/log_cpo_approx_different_meshs_100m_mean_density.png", dpi = 600, height = 8, width = 8)
