...
for (resp in unique(responses)) {
  ...
  list_mesh_res_sub <- future_map(
    ...
    function(i) {
      ...
      formula <- as.formula(paste0(resp, " ~ s(mean_density_km2_scaled, k = 3) +
      ...
      cv_model_id <- paste0("cv_", resp, "_", mesh_id, "_1000m_mean_density_smoothed")
...
fwrite(dt_mesh_res_fin, "builds/model_outputs/cv_mesh_selection_sdmtmb_results_1000m_mean_density_smoothed.csv")
...
p_loglik
ggsave(plot = p_loglik, "builds/plots/supplement/sum_loglik_different_meshs_1000m_mean_density_smoothed.png", dpi = 600, height = 4, width = 8)

### 4 - Best Mesh ------------------

dt_mesh_res_fin <- fread("builds/model_outputs/cv_mesh_selection_sdmtmb_results_1000m_mean_density_smoothed.csv")
...
best_mesh_res_list <- future_map(1:nrow(dt_best_mesh),
                                ...
                                s(n_deposition_scaled, k = 3)"))
                                   ...
                                   model_id = paste0(resp, "_best_mesh_1000m_mean_density_smoothed")
...
summary(dt_res)
fwrite(dt_res, "builds/model_outputs/sdmtmb_results_1000m_mean_density_smoothed.csv")
