...
corr_res <- round(cor(dt_corr_res), 2)
ggcorrplot(corr_res, hc.order = FALSE, type = "lower",
           lab = TRUE)
...
vars <- c("mean_density_km2_scaled",
          ...
          "prec_change_scaled", 
          "n_deposition_scaled", 
          "density_trend_estimate_scaled*mean_density_km2_scaled",
          "months_severe_drought_scaled*mean_density_km2_scaled",
          "fire_frequency_scaled*mean_density_km2_scaled", 
          "mat_change_scaled*mean_density_km2_scaled", 
          "prec_change_scaled*mean_density_km2_scaled", 
          "n_deposition_scaled*mean_density_km2_scaled")
...
p_aic
ggsave(plot = p_aic, "builds/plots/supplement/aic_univariate_models_100m_mean_density.png", dpi = 600, height = 6, width = 9)
# 
...
mesh_res_list <- future_map(unique(responses),
                            ...
                            function(resp) {
                              ...
                              for (i in 1:nrow(mesh_grid)) {
                                ...
                                formula <- as.formula(paste0(resp, " ~ density_trend_estimate_scaled*mean_density_km2_scaled +
          months_severe_drought_scaled*mean_density_km2_scaled +,
          fire_frequency_scaled*mean_density_km2_scaled + 
          mat_change_scaled*mean_density_km2_scaled + 
          prec_change_scaled*mean_density_km2_scaled + 
          n_deposition_scaled*mean_density_km2_scaled"))
                                ...
                                model_id = paste0(resp, "_", mesh_id, "_100m_mean_density_with_interactions")
...
summary(dt_mesh_res)
fwrite(dt_mesh_res, "builds/model_outputs/sdmtmb_results_100m_mean_density_with_interactions.csv")
...
p_covs <- dt_mesh_res %>% 
  ...
  theme(legend.position = "none", 
        ...
        strip.background = element_rect(fill = "linen", color = "linen"))
p_covs
ggsave(plot = p_covs, "builds/plots/supplement/cov_estimates_different_meshs_100m_mean_density_with_interactions.png", dpi = 600, height = 12, width = 12)
...
p_est <- dt_mesh_res %>% 
  ...
  theme(legend.position = "none", 
        ...
        strip.background = element_rect(fill = "linen", color = "linen"))
p_est
ggsave(plot = p_est, "builds/plots/cov_estimates_best_mesh_100m_mean_density_with_interactions.png.png", dpi = 600, height = 6.5, width = 10)
...
p_cpo <- dt_mesh_res %>% 
  ...
  theme(legend.position = "right", 
        ...
        strip.background = element_rect(fill = "linen", color = "linen"))
p_cpo
ggsave(plot = p_cpo, "builds/plots/supplement/log_cpo_approx_different_meshs_100m_mean_density_with_interactions.png.png", dpi = 600, height = 8, width = 8)
