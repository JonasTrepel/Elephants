...
dt_1000m <- dt_pred_smooth %>% 
  mutate(scale = "km2", 
         ...
         var_clean = case_when(
           ...
           var_name == "mean_density_km2_scaled" ~ "Mean Elephant Density",
...
fwrite(dt_1000m, "builds/model_outputs/mean_density_sdmtmb_1000m_predictions.csv")
...
dt_long <- dt_mod %>% pivot_longer(
  ...
  mutate(var_clean = case_when(
    ...
    var_name == "mean_density_km2" ~ "Mean Elephant Density",
...
p_smooth_points
ggsave(plot = p_smooth_points, "builds/plots/mean_density_1000m_model_predictions_and_points_smooth.png", dpi = 900, height = 5, width = 9)
...
ggsave(plot = p_smooth, "builds/plots/mean_density_1000m_model_predictions_smooth.png", dpi = 900, height = 6, width = 9)
