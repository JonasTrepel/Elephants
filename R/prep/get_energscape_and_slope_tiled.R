...
tile_template <- rast(ext = ext, resolution = 100000, crs = moll_crs) #100 km tiles

tiles <- getTileExtents(dem_trans_r, tile_template, buffer=2)

plan(multisession, workers = 4)

dt_res <- future_map(1:nrow(tiles), function(i){
  
  dem_trans_r <- terra::rast("data/spatial_data/covariates/raster/nasa_dem_30m_mollweide.tif")

  dem_tile <- terra::crop(dem_trans_r, terra::ext(tiles[i,])) 
