...
spei_binary_extreme <- spei_ic$map(function(img) {
  img$lte(-2)$rename("binary")$copyProperties(img, img$propertyNames()) #values lower than 2 indicate extreme droughts 
})
...
drive_download(file = filename_spei_extreme, path = "data/spatial_data/covariates/raster/spei_months_extreme_drought_2001_2025.tif", overwrite = TRUE)
googledrive::drive_rm(unique(drive_files_spei_extreme$name))
googledrive::drive_rm("rgee_backup_spei_extreme")

spei_extreme_r <- rast("data/spatial_data/covariates/raster/spei_months_extreme_drought_2001_2025.tif")
plot(spei_extreme_r)
